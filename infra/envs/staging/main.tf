...
module "network" {
  ...
  # Single-AZ in staging to halve interface-endpoint hourly cost. Dev/prod
  # leave this unset and keep the module default of 2 AZs.
  az_count = 1
  tags     = local.tags
}
...
module "cognito" {
  ...
  #
  # Staging deliberately leaves these blank — no Microsoft SSO in staging,
  # local Cognito users only.
