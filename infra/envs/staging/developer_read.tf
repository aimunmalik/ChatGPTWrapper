...
data "aws_iam_policy_document" "developer_read" {
  # CloudWatch Logs: read events from every anna-chat-${var.env}-* log group.
  # This is the big one — it's what `aws logs tail` + `filter-log-events` need.
  statement {
    sid    = "LogsRead"
    effect = "Allow"
    actions = [
  ...
  # DDB: read-only on anna-chat-${var.env}-* tables so we can inspect
  # conversation / message / kb state when debugging. No write actions.
  statement {
    sid    = "DynamoDBRead"
    effect = "Allow"
    actions = [
...
#     --policy-arn arn:aws:iam::<account>:policy/anna-chat-${var.env}-developer-read
#
# Or click through the AWS console → IAM → Users → lucent-dev → Add
# permissions → Attach policies → anna-chat-${var.env}-developer-read.
