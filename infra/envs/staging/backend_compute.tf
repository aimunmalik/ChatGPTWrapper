...
# GuardDuty allows only ONE detector per account per region, and the dev/prod
# env already created it. Reference the existing detector so staging reuses it
# instead of trying to create a duplicate (which would fail at apply).
...
module "attachments" {
  ...
  # Account-wide GuardDuty detector already exists (dev/prod created it); pass
  # its ID so the module skips creation rather than colliding on a second one.
