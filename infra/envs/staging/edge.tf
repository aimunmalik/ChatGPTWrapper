...
module "edge" {
  ...
  # Staging has NO custom domain — it serves on the CloudFront default
  # d*.cloudfront.net domain only. Empty aliases + empty cert ARN make the
  # edge module fall back to the CloudFront default certificate (see the
  # viewer_certificate block in modules/edge/main.tf). There is therefore
  # no domain.tf / ACM certificate in this environment.
